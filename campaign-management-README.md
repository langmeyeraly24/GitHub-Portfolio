...
- **The highest-leverage GOTV cell: Strong R + Low turnout** — 26 counties of existing R voters who aren't showing up. Cheapest per-vote mobilization target.
...
```
CMI Campaign Plan/
├── georgia_targeting_eda.ipynb      # Main EDA and targeting analysis
...
Data/
├── georgia voter + election data/   # Raw OpenElections + SOS files
...
The `images/targeting_viz_package/` folder contains 11 charts used in the final campaign plan:
...
- **Quarto:** voter targeting writeup (`voter_targeting_and_scoring.qmd`)

---

## Context

This work was completed for the **DCCC-affiliated Campaign Management Institute**, a competitive program that places participants on real campaign teams. The analytical framework (partisan blend scoring, lean bucketing, turnout tier modeling) reflects standard professional targeting methodology used in competitive Senate races.
